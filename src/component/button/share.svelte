  ...
  <div class="relative" bind:this={menu}>
	<div>
	  <button
		on:click={() => (show = !show)}
	  >
	  <Icon icon="cil:share-alt" color="#626262" width="20" height="20" />
	  </button>
	  ...
	  {#if show}
		<div
		  ...
		  class="absolute right-0 bottom-8 w-8 py-2 mt-1 bg-gray-200
			rounded shadow-md flex space-y-3 items-center flex-col"
		>
		<a href={url_whatsapp} class="block">
			<Icon icon="logos:whatsapp" width="15" height="15"/>
		</a>
		<a href={url_telegram}>
			<Icon icon="bx:bxl-telegram" color="#40b3e0" width="15" height="15" />
		</a>
		<a href={url_facebook}>
			<Icon icon="bi:facebook" color="#1877f2" width="15" height="15" />
		</a>
		<a href={url_twitter}>
			<Icon icon="logos:twitter" color="#40b3e0" width="15" height="15" />
		</a>
		</div>
	  {/if}
	</div>
